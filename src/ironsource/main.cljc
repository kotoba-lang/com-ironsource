(ns ironsource.main
  "Kotodama WASM entrypoint for the ironSource clean-room actor (L4) — Clojure port.
  ...
  facts (`ironsource.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ironsource")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ironsource.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ironsource-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ironsource-compat/ready)
